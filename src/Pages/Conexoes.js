...
export default function Conexoes() {
    ...
    async function getVacinas() {
        setLoading(true);
        try {
            const response = await fetch('http://10.139.75.101:5251/api/Vacinas/GetAllVacinas');
    ...
    async function getAvaliacao() {
        setLoading(true);
        try {
            const response = await fetch('http://10.139.75.101:5251/api/Avaliacaos/GetAllAvaliacaos');
            const data = await response.json();
            setAvaliacao(data);
        } catch (err) {
            setError(true);
